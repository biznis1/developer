...
return $this->makeTag('p',$gr['val'],$ops);
	...
	function heading($gr)
	{
		$par=array('name'=>'','id'=>'','class'=>'','style'=>'');
$ops=array_intersect_key($gr,$par);
return $this->makeTag('heading',$gr['val'],$ops);
	}
   ...
   return $this->makeTag('body',$gr['val'],$ops);
	...
	//http://192.168.2.61/dev/html/block/html.php?val=Proud%20to%20serve%20Who%27s%20Who%20of%20India&id=business&name=parpatganj
   ...
   return $this->makeTag('header',$gr['val'],$ops);
		  ...
		  function iframe($gr)
	{
		$par=array('name'=>'','id'=>'','src'=>'','height'=>'','width'=>'');
		$ops=array_intersect_key($gr,$par);
		 return $this->maketag('iframe',$gr['val'],$ops);
		
	}
	//http://192.168.2.61/dev/html/block/html.php?val=Fallback%20text%20for%20non-supported%20browsers&src=http://cssreference.io&height=350&width=350
		
function h1($gr)
	{
		$par=array('name'=>'','id'=>'','class'=>'','style'=>'');
   $ops=array_intersect_key($gr,$par);
   return $this->make($gr['tagName'],$gr['val'],$ops);
	}
	//http://192.168.2.61/dev/html/block/html.php?val=Proud%20to%20serve%20Who%27s%20Who%20of%20India&id=business&name=parpatganj	
   ...
   return $this->makeTag('adddress',$gr['val'],$ops);
	...
	function fieldset($gr)
	{
		$par=array('name'=>'','id'=>'','class'=>'','style'=>'');
      $ops=array_intersect_key($gr,$par);
      return $this->makeTag('fieldset',$gr['val'],$ops);
   ...
   return $this->makeTag('optgroup',$gr['label'],$ops);	
	...
	//http://192.168.2.61/dev/html/block/html.php?label=south%20africa
   ...
   return $this->makeTag('progress',$gr['max'],$ops);	
	...
	//http://192.168.2.61/dev/html/block/html.php?val=0.7&max=1
   ...
   return $this->maketag('select',$gr['name'],$ops);	
	...
	//	http://192.168.2.61/dev/html/block/html.php?name=countries
	function summary($gr)
	{
		$par=array('name'=>'','id'=>'','class'=>'','style'=>'','value'=>'','max'=>'');
		$ops=array_intersect_key($gr,$par);
	...
	//http://192.168.2.61/dev/html/block/html.php?id=tar&val=%20mehak
	function video($gr)
	{
